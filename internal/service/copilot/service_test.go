...
func TestServiceStopsAfterMaxToolRounds(t *testing.T) {
	...
	case <-done:
	case <-time.After(2 * time.Second):
		t.Fatal("tool loop did not stop")
	}
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
	if resp == nil || strings.TrimSpace(resp.Reply) == "" {
		t.Fatal("must return text after max tool rounds")
	}
	if provider.callCount() > MaxToolRounds+1 {
		t.Fatalf("provider Chat calls = %d, want <= %d", provider.callCount(), MaxToolRounds+1)
	}
	if schema.listTablesCalls() > MaxToolRounds {
		t.Fatalf("ListTables calls = %d, want <= %d", schema.listTablesCalls(), MaxToolRounds)
	}
}
