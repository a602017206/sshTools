...
// CloseCallback is called when a session exits from the remote/local side.
...
func (s *SessionService) ConnectSSH(sessionID, host string, port int, user, authType, authValue, passphrase string, cols, rows int, outputCallback OutputCallback, closeCallback CloseCallback) error {
	...
	err = s.sessionManager.StartShell(sessionID, cols, rows, outputCallback, closeCallback)
	if err != nil {
...
func (s *SessionService) ConnectLocalShell(sessionID string, shellType string, cols, rows int, outputCallback OutputCallback, closeCallback CloseCallback) error {
	_, err := s.sessionManager.CreateLocalSession(sessionID, shellType)
	if err != nil {
		return fmt.Errorf("failed to create local session: %w", err)
	}

	err = s.sessionManager.StartLocalShell(sessionID, cols, rows, outputCallback, closeCallback)
	if err != nil {
		s.sessionManager.CloseSession(sessionID)
