...
func (sm *SessionManager) StartShell(sessionID string, cols, rows int, onOutput func([]byte), onClose func(error)) error {
	sm.mu.Lock()
		...
		for {
			...
			default:
				...
				if err != nil {
					...
					shouldNotify, closeErr := sm.closeSession(sessionID, false)
					if closeErr != nil {
						fmt.Printf("Failed to cleanup session %s: %v\n", sessionID, closeErr)
					}
...
func (sm *SessionManager) closeSession(sessionID string, requestedByClient bool) (bool, error) {
	...
	shouldNotify := !requestedByClient
...
func (sm *SessionManager) StartLocalShell(sessionID string, cols, rows int, onOutput func([]byte), onClose func(error)) error {
	sm.mu.Lock()
	managed, exists := sm.sessions[sessionID]
	if !exists {
		sm.mu.Unlock()
		...
		for {
			...
			default:
				...
				if err != nil {
					...
					shouldNotify, closeErr := sm.closeSession(sessionID, false)
					if closeErr != nil {
						fmt.Printf("Failed to cleanup local session %s: %v\n", sessionID, closeErr)
					}
					if shouldNotify && onClose != nil {
						onClose(err)
					}
...
func (sm *SessionManager) CloseLocalSession(sessionID string) error {
	return sm.CloseSession(sessionID)
}
