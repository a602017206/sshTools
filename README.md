...
- ✅ **Base64 编解码工具**
  ...
  - 一键复制结果
- ✅ **加密解密工具**
  - MD5/SHA256/SHA512 哈希计算
  - AES-GCM/AES-CBC 加密解密
  - SM4-CBC 国密算法支持
  - 随机密钥/IV生成
- ✅ **时间戳转换工具**
  - Unix时间戳与日期时间互转
...
**开发工具集扩展**
- ✅ **Base64 编解码工具** - Base64编码和解码（已完成）
- ✅ **时间戳转换工具** - Unix时间戳与日期时间互转（已完成）
- ✅ **UUID 生成器** - 生成UUID v4（已完成）
- ✅ **Hash 计算器** - MD5/SHA256/SHA512哈希计算（已完成）
- ✅ **加密解密工具** - AES-GCM/CBC、SM4-CBC加密解密（已完成）
...
3. 输入文本或Base64字符串
4. 点击 **编码/解码** 按钮获取结果
5. 支持 `Ctrl+Enter` 快捷键执行

**使用加密解密工具**:
1. 选择 **加密解密** 工具
2. 选择算法类型：
   - **哈希**: MD5、SHA-256、SHA-512
   - **加密**: AES-GCM、AES-CBC、SM4-CBC
3. 对于加密算法：
   - 输入十六进制密钥（点击"随机生成"自动创建）
   - 输入IV/Nonce（点击"随机生成"自动创建）
   - 选择加密或解密模式
4. 输入待处理文本，点击执行
...
- [docs/README.md](./docs/README.md) - 文档目录和变更记录规范
- [docs/archive/DEVTOOLS_SUCCESS.md](./docs/archive/DEVTOOLS_SUCCESS.md) - 开发工具集部署成功报告归档
