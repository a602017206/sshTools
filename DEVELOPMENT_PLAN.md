...
- [x] 系统性能监控
  ...
  - [x] 磁盘使用统计
  - [x] 网络I/O流量
  - [x] 系统信息显示（OS版本、主机名、运行时间）
- [x] 开发工具集（DevTools）
  - [x] JSON格式化/验证/压缩/转义
  - [x] Base64编解码
  - [x] Hash计算（MD5/SHA256/SHA512）
  - [x] 加密解密（AES-GCM/AES-CBC/SM4-CBC）
  - [x] 时间戳转换
  - [x] UUID v4生成
  - [x] 可扩展的工具架构
...
- [x] 凭证管理
  - [x] AES-GCM加密存储实现
  - [x] 机器特征绑定的加密密钥
  - [ ] 系统密钥链集成（macOS Keychain、Windows Credential、Linux Secret Service）
  - [ ] SSH密钥生成
  - [ ] 密钥管理界面
- [ ] 跳板机支持
  - [ ] SSH ProxyJump
...
### 必备功能（MVP）- 全部完成 ✅
1. ✅ SSH连接（密码和密钥认证）
2. ✅ 终端模拟器（基于xterm.js，完整ANSI支持）
3. ✅ 连接配置保存（JSON配置文件）
4. ✅ 基础UI界面（Svelte + Wails）
5. ✅ 多会话管理（多标签页）
6. ✅ 密码加密存储（AES-256-GCM）

### 重要功能 - 大部分完成 ✅
1. ✅ SFTP文件传输（上传、下载、文件操作）
2. ✅ 系统监控（CPU、内存、磁盘、网络）
3. ✅ 开发工具集（JSON、Base64、Hash、加密、时间戳、UUID）
4. 🔸 SSH密钥管理（生成、管理界面）
5. 🔸 端口转发/隧道（本地/远程/动态转发）
6. 🔸 跳板机支持（ProxyJump）
7. 🔸 命令片段管理
8. 🔸 会话分屏
...
### 已完成 ✅
- **MVP版本**: ✅ 已完成（基础连接+终端+简单UI）
- **Beta版本**: ✅ 已完成（添加SFTP、系统监控、开发工具集）

### 进行中 🚀
- **正式版本**: 进行中（完善高级功能+测试+文档）
  - 端口转发/隧道功能
  - 跳板机支持
  - 命令片段管理
  - 会话分屏
