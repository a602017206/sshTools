...
<div class="h-full flex flex-col bg-white dark:bg-gray-800">
  ...
  <div class="flex-1 overflow-auto p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
    ...
    {:else}{#each categories as category}<section><h3 class="text-sm font-semibold mb-2">{category.icon} {category.label} ({objects[category.id]?.length || 0})</h3><div class="border border-gray-200 dark:border-gray-700 rounded max-h-72 overflow-auto">{#if errors[category.id]}<div class="px-3 py-2 text-xs text-red-600">加载失败：{errors[category.id]}</div>{:else}{#each objects[category.id] || [] as name}{#if category.id === 'tables'}<button type="button" class="block w-full px-3 py-2 text-sm text-left border-b border-gray-100 dark:border-gray-700 last:border-b-0 hover:bg-gray-50 dark:hover:bg-gray-700" on:click={() => openTableStructure(name)}>{name}</button>{:else}<div class="px-3 py-2 text-sm border-b border-gray-100 dark:border-gray-700 last:border-b-0">{name}</div>{/if}{/each}{#if !(objects[category.id] || []).length}<div class="px-3 py-2 text-xs text-gray-500">暂无{category.label}</div>{/if}{/if}</div></section>{/each}{/if}
  </div>
</div>
