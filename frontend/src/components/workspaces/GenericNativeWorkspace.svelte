<script>
  ...
  async function selectResource(resource, parent = '') {
    ...
    if (!workspace.canDescribe || !resource?.name || !window.wailsBindings || !sessionId) {
      if (resource?.name) selectedResource = resource.name;
      return;
    }
...
<section class="native-database-panel" aria-label={workspace.title}>
  ...
  <div class="native-database-panel__body">
          ...
          {#each resources as resource}
            <li class:expanded={expanded.has(resource.name)} class:selected={selectedResource === resource.name && !selectedParent}>
              {#if workspace.canExpand}
                <button
                  class="native-database-panel__resource"
                  type="button"
                  on:click={() => { toggleResource(resource); selectResource(resource); }}
                  aria-expanded={expanded.has(resource.name)}
                >
                ...
                <ul class="native-database-panel__children">
                  ...
                  {:else}
                    {#each childrenByParent[resource.name] || [] as child}
                      <li>
                        <button
                          class="native-database-panel__child"
                          type="button"
                          class:selected={selectedResource === child.name && selectedParent === resource.name}
                          on:click={() => selectResource(child, resource.name)}
                        >
    ...
    <aside class="native-database-panel__inspector">
      ...
      {#if loadingDetails}
        <p>正在读取对象详情…</p>
      {:else if details && workspace.canDescribe}
