<script>
  ...
  import {
    ...
    expandedGroupPathsForSearch,
    flattenVisibleGroupTree,
  } from '../lib/assetGroupTree.js';
  ...
  let showGroupContextMenu = false;
  let groupContextPath = '';
  let groupContextPosition = { x: 0, y: 0 };
  ...
  function openDbContextMenu(asset, event) {
    ...
    dbContextMenuPosition = resolveContextMenuPoint(event, { menuWidth: 176, menuHeight: 88 });
    showDbContextMenu = true;
  }

  function openGroupContextMenu(groupPath, event) {
    event.preventDefault();
    event.stopPropagation();
    showDbContextMenu = false;
    dbContextMenuAsset = null;
    groupContextPath = groupPath;
    groupContextPosition = resolveContextMenuPoint(event, { menuWidth: 176, menuHeight: 48 });
    showGroupContextMenu = true;
  }

  function handleNewConnectionInGroup() {
    const next = new Set(expandedGroups);
    for (const path of ancestorGroupPaths(groupContextPath)) {
      next.add(path);
    }
    expandedGroups = next;
    showGroupContextMenu = false;
    onAddClick?.(groupContextPath);
  }
  ...
  $: if (pendingExpandAssetId) {
    ...
    if (pendingAsset?.dbConnected) {
      if (assetSupportsJdbcSidebar(pendingAsset)) {
  ...
  function toggleDatabaseAsset(asset) {
    // Redis / ES / Kafka 等原生类型不走 JDBC 库树，连接后打开独立工作区。
    if (!assetSupportsJdbcSidebar(asset)) {
      dispatchDatabaseConnect(asset, true);
      return;
    }
  ...
  $: searchExpandedPaths = expandedGroupPathsForSearch(groupedFilteredAssets, searchTerm);
  $: visibleExpandedGroups = (() => {
    if (!searchExpandedPaths.length) return expandedGroups;
    const next = new Set(expandedGroups);
    for (const path of searchExpandedPaths) next.add(path);
    return next;
  })();
  $: groupForest = buildAssetGroupForest(groupedFilteredAssets);
  $: visibleGroupRows = flattenVisibleGroupTree(groupForest, visibleExpandedGroups);

  function treePad(depth) {
...
<div class="h-full flex flex-col" on:click={handleClickOutside}>
  ...
  <div class="flex-1 overflow-y-auto scrollbar-thin py-2">
    {#each visibleGroupRows as row (row.kind === 'group' ? `g:${row.node.path}` : `a:${row.asset.id}`)}
      {#if row.kind === 'group'}
        <button
          type="button"
          on:click={() => toggleGroup(row.node.path)}
          on:contextmenu={(event) => openGroupContextMenu(row.node.path, event)}
          on:selectstart|preventDefault
          class="ops-group-row w-[calc(100%-1rem)] flex items-center gap-2 pr-3 py-1.5 mx-2 rounded-md cursor-pointer transition-colors text-left"
          class:mt-2={row.depth === 0}
          style={`padding-left: ${treePad(row.depth)}`}
        >
          {#if visibleExpandedGroups.has(row.node.path)}
          ...
          <span class="text-[11px] font-semibold uppercase tracking-wide">{row.node.name}</span>
          <span class="ml-auto text-[10px] opacity-70">{row.node.assetCount}</span>
        </button>
      {:else}
        {@const asset = row.asset}
        <div style={`padding-left: ${treePad(row.depth)}`} class="mx-2">
          <div
          ...
          {#if assetSupportsJdbcSidebar(asset) && expandedDatabaseAssets.has(asset.id)}
...
{#if showDbContextMenu && dbContextMenuAsset}
  <div
    class="ops-flyout fixed z-[120] w-44 rounded-xl"
...
{#if showGroupContextMenu && groupContextPath}
  <div
    class="ops-flyout fixed z-[120] w-44 rounded-xl"
    style={`top: ${groupContextPosition.y}px; left: ${groupContextPosition.x}px;`}
    role="menu"
    use:portalToBody
  ...
  >
    <button
      type="button"
      class="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
      on:click={handleNewConnectionInGroup}
    >
      <svg class="w-4 h-4 accent-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
      </svg>
